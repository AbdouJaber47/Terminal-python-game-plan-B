...
recharger = pygame.mixer.Sound('reload.wav')
ARMES = {
    "AK-47": (53, 0.11, 4, 60, pygame.mixer.Sound('ak.ogg')),
    "Minigun": (13, 0.29, 4, 120, pygame.mixer.Sound('machinegun.ogg')),
    "Lance-roquettes": (1000, 7.028, 4, float('inf'), pygame.mixer.Sound('rpg.wav'))
}
...
class Joueur:
    def __init__(self, nom, arme, armure):
        ...
        self.arme = arme
        self.armure = armure
        self.degats, self.cadence, self.recharge, self.munitions_max, self.son = ARMES[self.arme]
...
while boss.pv > 0:
    ...
    for event in pygame.event.get():
        if event.type == pygame.USEREVENT:
            print("\n🎵Vous n'avez plus d'oxygene... Vous êtes mort !")
            exit()
    if boss.pv <= 0:
